...
void Debug::AnimStateMonitor() noexcept
{
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
        ...
        for (int b = 0; b < entity->getAnimationLayerCount(); b++) {
            ...
            std::vector<std::wstring> strings{ playername, clientblend, dispatchedsrc, dispatcheddst, blendin, cycle, prevcycle, weight, weightdelta, overlay, state, order, playbackrate };
